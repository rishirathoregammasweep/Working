...
For **code excerpts and a full storage matrix** (Redis keys, SQL tables, queues), see **§9**.
...
A **trigger** is a row in PostgreSQL table **`triggers`** that says: *when an incoming event matches this brand, this `event_type`, and all `conditions`, then start a **campaign send path** for that player using **`channels` from this row***. It does **not** store message copy; copy lives on the linked **`campaigns`** row (`campaign_id`). Triggers are evaluated inside **campaign-engine** after player state is updated and conversions are checked (`EventConsumerService` → `TriggerEvaluatorService`).

### What a matching trigger starts (systems and side effects)

End-to-end, a trigger match is the **decision** that kicks off the rest of the pipeline:

1. **In-memory / no extra DB read for the rule:** The evaluator already loaded the trigger row; it builds a **`MatchedTrigger`** (`trigger_id`, `campaign_id`, `channels[]`, `brand_id`, `player_id`, `event_type`).
2. **`CampaignPublisherService.publishCampaign`:** Loads **`campaigns`** by `campaign_id`, applies A/B and control-group logic, enriches the outbound payload with template fields from the campaign, optionally writes **`campaign_delivery_logs`** (only when **not** in the control group — for conversion attribution), and publishes JSON to RabbitMQ **`ge.campaigns`** / routing key **`campaigns.outbound.v1`** → queue **`ge.campaigns.outbound`**.
3. **channel-delivery:** Consumes that queue, applies throttles/suppression, renders templates, sends on the listed channels (email, SMS, push, etc.), and may emit analytics/webhooks.

**Other storage touched by trigger logic (not always on every event):**

- **Redis `player_state:…`:** Updated on every event *before* triggers run; conditions may read fields from this state.
- **Redis `seq:{brand_id}:{player_id}:{sequence_id}`:** Only for **sequential** triggers (`sequence_id` set); stores progress until the final step completes or the window resets.
- **AI engine (HTTP):** If any active trigger for that event type has conditions on `churn_score`, `vip_score`, or `rg_risk_score`, scores are fetched once; **RG block** clears all matches for that event.
- **Segment definitions + Redis:** If a condition uses `field === 'segment_id'`, `SegmentationService` evaluates membership.

**Not started by `triggers` rows:** **Journeys** use their own **`journeys.trigger_event_type`** and enrollment logic in parallel with trigger evaluation — related conceptually, different table and code path.

### PostgreSQL: `triggers` table — every column

| Column | Type (migration) | Purpose |
|--------|------------------|---------|
| **`id`** | `uuid`, PK, default `gen_random_uuid()` | Stable identifier for this trigger. Becomes **`trigger_id`** on outbound messages and in **`campaign_delivery_logs.trigger_id`** when a non–control-group send is logged. |
| **`brand_id`** | `varchar`, NOT NULL | Tenant scope. Evaluator loads triggers where `brand_id` matches **`EventEnvelope.brand_id`**. |
| **`name`** | `varchar`, NOT NULL | Human/admin label only; not used in evaluation. |
| **`event_type`** | `varchar`, NOT NULL | Which incoming event name this row applies to (must match **`EventEnvelope.event_type`** for the row to be considered). |
| **`conditions`** | `jsonb`, default `'[]'` | AND list of **`TriggerCondition`** objects (see below). Empty array means “no extra predicates” beyond brand/event/active. |
| **`campaign_id`** | `varchar`, NOT NULL | Which **`campaigns.id`** to load for templates, control group %, waterfall, conversion settings. No DB FK — allows creating triggers before the campaign exists. |
| **`channels`** | `varchar`, default `'email'` | Comma-separated list (e.g. `email,sms,push`). Parsed into an array on match; **only** this list defines channels for event-triggered sends (campaign’s own `channels` is not merged in for this path). |
| **`is_active`** | `boolean`, default `true` | If `false`, the row is ignored by the evaluator. Part of index **`idx_triggers_brand_event_active`**. |
| **`sequence_id`** | `varchar`, nullable | If **null**, this is a **single-event** trigger: match immediately when conditions pass. If **set**, this row is one **step** in a named sequence; Redis tracks progress until the last step completes. |
| **`sequence_position`** | `int`, default `1` | 1-based step index **within** `sequence_id`. Must align with event order logic in `TriggerEvaluatorService` (wrong order resets or waits). |
| **`sequence_total_steps`** | `int`, default `1` | How many steps the sequence has; the **final** step’s trigger row fires the campaign when all prior steps completed in order (within the window if set). |
| **`sequence_window_seconds`** | `int`, default `0` | Max seconds from first step to last; `0` means no time limit. If exceeded, sequence state is cleared and the match is not emitted. |
| **`created_at`** | `timestamp`, default `now()` | Audit: when the row was created. |
| **`updated_at`** | `timestamp`, default `now()` | Audit: last update (TypeORM `@UpdateDateColumn`). |

**Indexes (from migration):**

- **`idx_triggers_brand_event_active`** on `(brand_id, event_type, is_active)` — supports the evaluator’s lookup.
- **`idx_triggers_sequence`** on `(brand_id, sequence_id)` — useful for admin/listing sequential groups.

### JSON inside `conditions` (not separate columns)

Each element of **`conditions`** is a **`TriggerCondition`**:

| Field | Meaning |
|--------|---------|
| **`field`** | Name of value to test: a **`PlayerState`** property, **`EventEnvelope.payload`** key, **`segment_id`** (special), or **`churn_score` / `vip_score` / `rg_risk_score`** (AI). |
| **`op`** | `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`. |
| **`value`** | Scalar or string array (for `in` / `not_in`). |
| **`segment_id`** | When `field === 'segment_id'`, the UUID of the segment definition to test via **`SegmentationService.playerMatchesSegment`**. |

### Related PostgreSQL: `trigger_id` on other tables

| Table | Column | Role |
|--------|--------|------|
| **`campaign_delivery_logs`** | **`trigger_id`** | Set from the **`MatchedTrigger`** when logging a dispatch for **conversion attribution** (skipped for control-group assignments). |

**Synthetic IDs (not rows in `triggers`):** Scheduled bulk sends use a publisher payload with **`trigger_id`** like **`sched:{schedule_id}`** from `SchedulerService` — same outbound shape, different source than event triggers.
...
Each subsection includes **source excerpts** (with file paths and line ranges), a short **behavior** summary, and a **storage** table listing **PostgreSQL** tables, **Redis** keys/patterns, **RabbitMQ** exchanges/queues, **ClickHouse** tables, or **HTTP** calls. Use this as an implementation map for auditing and debugging.
...
```56:123:services/campaign-engine/src/conversions/conversion-tracker.service.ts
  async checkConversion(envelope: EventEnvelope): Promise<void> {
    ...
    // Use a single SQL query that joins delivery log + campaign filter.
    const now = new Date();
    ...
    // Only consider campaigns that track this event type
    const eligibleCampaigns = campaigns.filter((c) =>
      (c.conversion_event_types ?? []).includes(eventType),
    );
    if (eligibleCampaigns.length === 0) return;

    const revenue = this.extractAmount(envelope.payload);

    for (const campaign of eligibleCampaigns) {
      const log = recentLogs.find((l) => l.campaign_id === campaign.id);
      if (!log) continue;

      const attributionWindowHours = campaign.attribution_window_hours ?? 72;
      const hoursElapsed = (now.getTime() - log.sent_at.getTime()) / 3_600_000;
      if (hoursElapsed > attributionWindowHours) continue;

      // Use INSERT … ON CONFLICT DO NOTHING to prevent duplicate conversions atomically.
      // The unique index on (brand_id, campaign_id, player_id, converted_event_type) enforces this.
      try {
        ...
        this.logger.info(
          { campaignId: campaign.id, playerId, eventType, hoursElapsed: hoursElapsed.toFixed(1) },
          'Conversion attributed',
        );
      } catch (err) {
        this.logger.warn({ err, campaignId: campaign.id, playerId }, 'Conversion insert failed');
      }
    }
  }
```
...
```112:197:services/campaign-engine/src/triggers/trigger-evaluator.service.ts
  /**
   * Evaluates a sequential trigger step. Stores progress in Redis.
   * Returns true only when the final step in the sequence is completed within the window.
   *
   * Redis key: seq:{brand_id}:{player_id}:{sequence_id}
   * Redis value: JSON array of completed SequenceStep entries
   */
  private async evaluateSequence(
    ...
    if (raw) {
      try {
        steps = JSON.parse(raw) as SequenceStep[];
      } catch {
        // Corrupted Redis value — treat as a fresh start and overwrite
        this.logger.warn({ key }, 'Corrupt sequence state in Redis — resetting');
        await this.redis.del(key);
      }
    }

    // Determine which step we expect next
    const expectedPosition = steps.length + 1;

    if (trigger.sequence_position !== expectedPosition) {
      // This event matches a later step — ignore until earlier steps are done
      // OR this event matches an earlier step — reset the sequence
      if (trigger.sequence_position === 1) {
        // Restart the sequence from step 1
        const newSteps: SequenceStep[] = [
          { event_type: envelope.event_type, occurred_at: envelope.occurred_at, position: 1 },
        ];
        await this.persistSequence(key, newSteps, trigger.sequence_window_seconds);
      }
      return false;
    }

    // This is the expected next step — advance the sequence
    steps.push({
      event_type: envelope.event_type,
      occurred_at: envelope.occurred_at,
      position: trigger.sequence_position,
    });

    // Check if the time window has been exceeded (if a window is configured)
    if (trigger.sequence_window_seconds > 0 && steps.length > 1) {
      const firstStepTime = new Date(steps[0].occurred_at).getTime();
      const currentTime = new Date(envelope.occurred_at).getTime();
      const elapsedSeconds = (currentTime - firstStepTime) / 1000;

      if (elapsedSeconds > trigger.sequence_window_seconds) {
        this.logger.debug(
          { key, elapsedSeconds, windowSeconds: trigger.sequence_window_seconds },
          'Sequence window exceeded — resetting',
        );
        await this.redis.del(key);
        return false;
      }
...
```70:116:services/channel-delivery/src/send-throttle/send-throttle.service.ts
  async checkAndRecord(
    ...
    try {
      ...
      this.logger.debug(
        { brandId, playerId, channel, count, latencyMs: Date.now() - startMs },
        'Throttle: daily cap checked',
      );
      if (count > this.dailyCap) {
        this.logger.debug(
          { brandId, playerId, channel, count, dailyCap: this.dailyCap },
          'Throttle: daily cap exceeded',
        );
        // Decrement — we incremented before checking, roll back
        await this.redis.decr(capKey);
        return { allowed: false, reason: 'daily_cap_exceeded' };
      }
    } catch (err) {
      // Redis unavailable — fail open (allow send) to avoid blocking all delivery
      this.logger.warn(
        { err: String(err), latencyMs: Date.now() - startMs },
        'Throttle: Redis unavailable — allowing send',
      );
    }
...
**Failed delivery retries (`DeliveryRetryService`):** **INSERT** into PostgreSQL table **`delivery_attempts`** for later cron replay.
...
| `SuppressionService` | **PostgreSQL** table **`suppressions`** | **SELECT** to block sends when player/email/channel is suppressed. |
...
| Retries | **PostgreSQL** | **INSERT** pending row in **`delivery_attempts`**. |
...
## 11. Related code (quick reference)
