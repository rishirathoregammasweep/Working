...
# Event → trigger flow (hierarchical diagram)

This page describes the same pipeline as [trigger-explanation.md](./trigger-explanation.md), but uses a **top-down tree** (mind-map style) with **color-coded branches** instead of UML.

**How to read it:** Start at the root circle, then follow each main branch. **Process pipeline** is the vertical journey of one event. **Data & persistence** lists **PostgreSQL**, **Redis**, and **ClickHouse** touchpoints. **Messaging** covers RabbitMQ. **Outbound** lists delivery paths and follow-up analytics.

---

## Main diagram — hierarchical tree

```mermaid
graph TD
  %% Root (circle, like the reference “Types of Diagrams” node)
  ROOT((GammaEngage\nEvent → trigger → delivery))

  %% Level 2 — main categories (rounded / stadium shapes)
  ROOT --> PROC([Process pipeline])
  ROOT --> DATA([Data & persistence])
  ROOT --> MSG([Messaging])
  ROOT --> OUT([Outbound & channels])

  %% Process branch — green theme (stages top to bottom in text order)
  PROC --> P1[HTTP ingest — event-ingestion]
  PROC --> P2[Idempotency + optional enrichment]
  PROC --> P3[Publish — ge.events.raw.v1]
  PROC --> P4[Consume — campaign-engine EventConsumer]
  PROC --> P5[Player state + snapshot + conversions + journeys]
  PROC --> P6[TriggerEvaluator — conditions + sequences]
  PROC --> P7[CampaignPublisher — A/B, control group, templates]
  PROC --> P8[Publish — ge.campaigns / outbound]
  PROC --> P9[CampaignConsumer — render, policy, suppression]
  PROC --> P10[Dispatch — waterfall or concurrent]

  %% Data branch — orange: split into three stores
  DATA --> DPG[(PostgreSQL)]
  DATA --> DRS[(Redis)]
  DATA --> DCH[(ClickHouse)]

  DPG --> PG1[campaign_player_snapshots]
  DPG --> PG2[triggers, campaigns — reads]
  DPG --> PG3[campaign_delivery_logs, campaign_conversions]
  DPG --> PG4[journey_enrollments]
  DPG --> PG5[email tracking tokens]

  DRS --> R1[idempotency keys — ingest]
  DRS --> R2[player_state — engine]
  DRS --> R3[seq:* — sequential triggers]
  DRS --> R4[send throttle — delivery]

  DCH --> CH1[events_raw — ingest]
  DCH --> CH2[player_state — optional ingest aggregates]
  DCH --> CH3[events_raw_buffer — campaign.dispatched, opens/clicks]

  %% Messaging — yellow
  MSG --> Q1[Queue ge.events.raw.v1]
  MSG --> Q2[Exchange ge.campaigns]
  MSG --> Q3[Queue ge.campaigns.outbound]
  MSG --> Q4[Routing key campaigns.outbound.v1]

  %% Outbound — purple / delivery leaf nodes
  OUT --> C1[Email]
  OUT --> C2[SMS]
  OUT --> C3[Mobile push]
  OUT --> C4[Web push]
  OUT --> C5[WhatsApp]
  OUT --> C6[Popup / SSE]
  OUT --> C7[Webhooks + analytics forward]

  %% Styling — approximate reference palette: green / orange / yellow / purple
  classDef rootStyle fill:#b8d4e8,stroke:#2c5282,stroke-width:2px,color:#1a202c
  classDef processCat fill:#c8e6c9,stroke:#2e7d32,stroke-width:1px,color:#1b5e20
  classDef dataCat fill:#ffe0b2,stroke:#e65100,stroke-width:1px,color:#bf360c
  classDef msgCat fill:#fff9c4,stroke:#f9a825,stroke-width:1px,color:#f57f17
  classDef outCat fill:#e1bee7,stroke:#6a1b9a,stroke-width:1px,color:#4a148c
  classDef leaf fill:#fafafa,stroke:#616161,stroke-width:1px,color:#212121

  class ROOT rootStyle
  class PROC,P1,P2,P3,P4,P5,P6,P7,P8,P9,P10 processCat
  class DATA,DPG,DRS,DCH,PG1,PG2,PG3,PG4,PG5,R1,R2,R3,R4,CH1,CH2,CH3 dataCat
  class MSG,Q1,Q2,Q3,Q4 msgCat
  class OUT,C1,C2,C3,C4,C5,C6,C7 outCat
```

> **Rendering:** GitHub and many Markdown previews render Mermaid. If colors do not appear, the structure (root → four branches → leaves) is unchanged.

---

## Compact tree — storage only

Use this when you only care **where Postgres, Redis, and ClickHouse** appear relative to the three pipeline stages.
...
graph TD
  ...
  ROOT --> ING([Ingest])
  ROOT --> ENG([Campaign engine])
  ROOT --> DEL([Channel delivery])

  ING --> ING_R[Redis — idempotency]
  ING --> ING_C[ClickHouse — events_raw, optional player_state]

  ENG --> ENG_R[Redis — player_state, seq:*]
  ENG --> ENG_P[Postgres — snapshots, triggers, campaigns, logs, conversions, journeys]

  DEL --> DEL_P[Postgres — contact, suppression, tracking]
...
| Color (main diagram) | Branch | Meaning |
|---------------------|--------|---------|
| Green | Process pipeline | Ordered steps from HTTP ingest through trigger evaluation to dispatch. |
| Orange | Data & persistence | Concrete tables / keys / CH tables touched or updated. |
| Yellow | Messaging | RabbitMQ queues and routing used between services. |
| Purple | Outbound & channels | End delivery and side effects (webhooks, analytics). |
